...
export class AdminController {
    ...
    async addNewService(@Body() serviceData: AddNewServiceDto) {
        try {
            const result = await this.servicesService.addNewService(
                serviceData
            );
            return {
    ...
    async updateServiceById(@Body() serviceData: UpdateServiceDto) {
        try {
            const result = await this.servicesService.updateServiceById(
                serviceData
            );
            return {
                status: HttpStatus.OK,
    ...
    async deleteServiceById(@Body() serviceId: DeleteServiceDto) {
        try {
            const result = await this.servicesService.deleteServiceById(
                serviceId.serviceId
            );
            return {
                status: HttpStatus.OK,
    ...
    async createRoomType(@Body() roomTypeInput: AddNewRoomTypeDto) {
        try {
            const result = await this.roomTypeService.createRoomType(
                roomTypeInput
            );
            return {
                status: HttpStatus.OK,
    ...
    @ApiOperation({
        summary: "Update room info: change room number or change type of room.",
    })
    @Patch("room/:roomId")
    async updateRoom(
    ...
    @ApiOperation({
        summary: "Delete user by condition, only allow: userName, email, id.",
    })
    ...
    async getRoomDetailBill(@Param("id") id: string) {
        try {
            ...
            const result = await this.roomDetailService.computeBill(
                roomDetailId
            );
            return {
                status: HttpStatus.OK,
    ...
    @ApiOperation({ summary: "Deactive room by Id." })
    @Patch("room_deactive/:id")
    async deactiveRommById(@Param("id") id: string) {
        try {
            const roomId = parseInt(id);
            const result = await this.roomService.deactiveRommById(roomId);
            return {
                status: HttpStatus.OK,
                error: 0,
    ...
    @ApiOperation({ summary: "Soft delete the room" })
    @Delete("room/:id")
    async deleteRoomById(@Param("id") id: string) {
        try {
            const roomId = parseInt(id);
            const result = await this.roomService.deleteRoomById(roomId);
            return {
                status: HttpStatus.OK,
                error: 0,
                message: "Delete room successfully.",
            ...
            console.error("admin.controller.ts deleteRoomById: ", err);
            return {
                status: err.status,
                error: 1,
                message:
